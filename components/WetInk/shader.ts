 ...
 * the glyphs and the colour is chosen here. uProgress is the drying ramp, which starts at 1 for
...
void main() {
  ...
  a += (texture2D(uTex, uv + dir * 1.4 * px).a - a) * wave * 0.8;

  vec3 col = mix(uInk, uInk * 0.72 + uAccent * 0.14, wet * 0.7);
  col = mix(col, uAccent, exp(-dist * 0.022) * uHover * 0.6);
